...
title: SSR 与服务端框架
order: 7
description: 在 Nuxt、Astro、Vite SSR 等环境中使用 Roost Design UI。
---

# SSR 与服务端框架

Roost Design UI 面向 **Vue 3** 的 SSR 场景做了兼容（推荐 3.5 及以上）：服务端不会访问 `document` / `window`，组件实例 id 在服务端与客户端保持一致，命令式 API（`toast` / `message` / `confirm`）在服务端自动跳过。
...
| 项 | 建议 |
| --- | --- |
| 样式 | 在应用入口或框架配置中引入 `@roost-design/ui/styles.css` |
...
| 按需导入 | 配合 `@roost-design/ui/resolver` 与 `unplugin-vue-components` 减小体积 |
| 浮层 | Dialog / Select / Tooltip 等使用 Vue `Teleport`；SSR 可输出占位，交互在客户端水合后生效 |

## Nuxt 3

推荐使用官方模块 **`@roost-design/nuxt`**（同仓库 `packages/nuxt`）。
...
- 引入 `@roost-design/ui/styles.css`
- 将 `@roost-design/ui` 加入 `build.transpile`
...
import { RdConfigProvider } from '@roost-design/ui/config-provider'
import '@roost-design/ui/styles.css'
...
createApp(/* ... */)
  .use(createRoostDesign({ components: false }))
  .mount('#app')
```

构建时确保 `@roost-design/ui` 进入 `ssr.noExternal`（或 `ssr: { noExternal: ['@roost-design/ui'] }`），以便正确处理 `.vue` 与 CSS 副作用。

## 其他 Vue SSR 方案

| 方案 | 说明 |
| --- | --- |
| **Quasar (SSR mode)** | 在 `quasar.config` 的 `build.transpileDependencies` 中加入 `@roost-design/ui`，入口引入样式 |
| **vite-plugin-ssr / vike** | 与 Vite SSR 相同：服务端渲染 + 客户端 `createRoostDesign({ components: false })` |
...
- 全量 E2E SSR 回归仍在完善；若遇到 hydration 警告，请提供最小复现并 [提 Issue](https://github.com/roost-design/roost-design-ui/issues)。
