...
<template>
  <RdConfigProvider class="site-config" :locale="componentLocale" :global-density="false">
...
:root {
  ...
  --docs-ink: color-mix(in srgb, var(--rd-color-text) 92%, #041018);
  --docs-glow: color-mix(in srgb, var(--rd-color-primary) 55%, #22d3ee);
  --docs-panel: color-mix(in srgb, var(--rd-color-surface) 78%, transparent);
  --docs-edge: color-mix(in srgb, var(--rd-color-border) 70%, transparent);
...
input:focus-visible,
a:focus-visible {
  outline: 3px solid color-mix(in srgb, var(--rd-color-focus-ring) 40%, transparent);
  outline-offset: 3px;
}

.rd-autocomplete__input:focus-visible,
.rd-autocomplete__dropdown:focus-visible,
.rd-select:focus-visible,
.rd-icon-field input:focus-visible {
  outline: none;
  outline-offset: 0;
}
...
.site-atmosphere__glow--b {
  background: radial-gradient(circle, color-mix(in srgb, var(--rd-color-primary) 45%, #0ea5e9) 0%, transparent 70%);
  bottom: -12rem;
  height: 32rem;
  opacity: 0.28;
  right: -10rem;
