...
<template>
  <div class="mobile-sidebar-shell">
    ...
    >
      <MScrollbar :class="scrollClass ?? 'mobile-sidebar-shell__scroll'">
        <div
          :class="bodyClass"
          @click="onPanelClick"
        >
...
.mobile-sidebar-shell__panel > :deep(.m-scrollbar),
...
.mobile-sidebar-shell__panel > :deep(.m-scrollbar .m-scrollbar__wrap) {
...
.mobile-sidebar-shell__scroll :deep(.m-scrollbar__wrap) {
  overscroll-behavior: contain;
}
...
@media (max-width: 700px) {
  ...
  .mobile-sidebar-shell__panel {
    background: color-mix(in srgb, var(--m-color-surface) 96%, transparent);
