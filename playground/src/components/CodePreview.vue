    ...
    <details class="code-preview__code">
      <summary>
        ...
        <button
          ...
          @click="onCopy"
        >
          <RdIcon :name="copied ? 'check' : 'copy'" size="sm" />
          <span>{{ label }}</span>
        </button>
      </summary>
...
.code-preview__demo {
  align-items: center;
  background: color-mix(in srgb, var(--rd-color-primary) 4%, var(--rd-color-surface));
  display: flex;
  flex-wrap: wrap;
  gap: var(--rd-space-3);
  min-height: 7rem;
...
.code-preview__demo :deep(.rd-splitter) {
  align-self: stretch;
  flex: 1 1 100%;
  width: 100%;
}
...
.code-preview__copy {
  align-items: center;
  background: color-mix(in srgb, var(--rd-color-surface) 80%, transparent);
  border: 1px solid var(--rd-color-border);
