...
const READY = 'data-rd-copy-ready'
...
function mountCopyButton(
  ...
  if (parent.classList.contains('rd-code-block')) {
    wrapper = parent
  } else {
    wrapper = document.createElement('div')
    wrapper.className = 'rd-code-block'
    parent.insertBefore(wrapper, pre)
    wrapper.appendChild(pre)
  }

  let button = wrapper.querySelector<HTMLButtonElement>('.rd-code-block__copy')
  if (!button) {
    button = document.createElement('button')
    button.type = 'button'
    button.className = 'rd-code-block__copy'
