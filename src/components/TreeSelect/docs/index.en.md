...
<template>
  <RdTreeSelect v-model="value" :options="options" />
</template>
```
...
<template>
  <div style="display:flex;flex-wrap:wrap;gap:0.75rem">
    <RdTreeSelect v-model="a" size="small" :options="options" placeholder="Small" />
    <RdTreeSelect v-model="b" :options="options" placeholder="Default" />
    <RdTreeSelect v-model="c" size="large" :options="options" placeholder="Large" />
  </div>
</template>
```
