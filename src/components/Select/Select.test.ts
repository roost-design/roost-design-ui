...
describe('rdSelect', () => {
  it('associates its label and emits a typed selected value', async () => {
    const wrapper = mount(RdSelect, { props: { id: 'size', label: 'Size', options } })
  ...
  it('renders placeholder and invalid state, then supports keyboard selection', async () => {
    ...
    expect(trigger.classes()).toContain('rd-select--invalid')
  ...
  it('supports size and fluid props', () => {
    const wrapper = mount(RdSelect, { props: { options, size: 'small', fluid: true } })
    expect(wrapper.classes()).toContain('rd-select-field--fluid')
    expect(wrapper.get('[role="combobox"]').classes()).toContain('rd-select--small')
  })

  it('teleports the styled menu to body by default', async () => {
    const wrapper = mount(RdSelect, { props: { options, modelValue: 'sm' }, attachTo: document.body })
    await wrapper.get('[role="combobox"]').trigger('click')
    await nextTick()

    expect(document.body.querySelector('.rd-select__menu--teleported')).toBeTruthy()
    wrapper.unmount()
  })
  ...
  it('filters options by label and shows empty message', async () => {
    ...
    await wrapper.get('[role="combobox"]').trigger('click')
    await wrapper.get('.rd-select__filter').setValue('zzz')
    await nextTick()
    expect(wrapper.findAll('[role="option"]')).toHaveLength(0)
    expect(wrapper.get('.rd-select__empty').text()).toBe('暂无选项')

    await wrapper.get('.rd-select__filter').setValue('lar')
    await nextTick()
  ...
  it('shows empty message when options are empty', async () => {
    ...
    await wrapper.get('[role="combobox"]').trigger('click')
    expect(wrapper.get('.rd-select__empty').text()).toBe('没有可选内容')
  })
  ...
  it('renders removable tags and can collapse extras', async () => {
    ...
    expect(wrapper.get('.rd-select__tag-label').text()).toBe('Small')
    expect(wrapper.get('.rd-select__tag--more').text()).toBe('+1')
    await wrapper.get('.rd-select__tag-remove').trigger('click')
    expect(wrapper.emitted('update:modelValue')).toEqual([[[2]]])
  })
  ...
  it('shows loading copy and creates a tag option from the filter query', async () => {
    ...
    await wrapper.get('[role="combobox"]').trigger('click')
    await wrapper.get('.rd-select__filter').setValue('Brand new')
    await nextTick()
    const create = wrapper.get('.rd-select__option--create')
