...
import RdFileUpload from './FileUpload.vue'
...
async function pick(wrapper: ReturnType<typeof mount>, files: File[]) {
  const input = wrapper.find('.rd-fileupload__input')
...
describe('rdFileUpload', () => {
  it('emits select when files change', async () => {
    const wrapper = mount(RdFileUpload, { props: { mode: 'advanced' } })
    const file = makeFile()
  ...
  it('opens picker on choose click', async () => {
    const wrapper = mount(RdFileUpload)
    const input = wrapper.find('.rd-fileupload__input').element as HTMLInputElement
    const click = vi.spyOn(input, 'click').mockImplementation(() => undefined)
    await wrapper.find('.rd-fileupload__choose').trigger('click')
    expect(click).toHaveBeenCalled()
  })

  it('renders a drop zone when drag is enabled', async () => {
    const wrapper = mount(RdFileUpload, { props: { drag: true } })
    expect(wrapper.find('.rd-fileupload__dragger').exists()).toBe(true)
    expect(wrapper.find('.rd-fileupload__choose').exists()).toBe(false)
    expect(wrapper.text()).toContain('将文件拖到此处')
  })

  it('emits select when files are dropped', async () => {
    const wrapper = mount(RdFileUpload, { props: { drag: true, multiple: true } })
    const file = makeFile()
    await wrapper.find('.rd-fileupload__dragger').trigger('drop', {
  ...
  it('highlights the drop zone while dragging over', async () => {
    const wrapper = mount(RdFileUpload, { props: { drag: true } })
    const zone = wrapper.find('.rd-fileupload__dragger')
    await zone.trigger('dragover', { dataTransfer: { dropEffect: 'none' } })
    expect(zone.classes()).toContain('rd-fileupload__dragger--over')
    await zone.trigger('dragleave')
    expect(zone.classes()).not.toContain('rd-fileupload__dragger--over')
  })

  it('emits exceed when dropped files go past the limit', async () => {
    const wrapper = mount(RdFileUpload, { props: { drag: true, multiple: true, limit: 1 } })
    await wrapper.find('.rd-fileupload__dragger').trigger('drop', {
  ...
  it('emits exceed when appending files would pass the limit', async () => {
    const wrapper = mount(RdFileUpload, { props: { drag: true, multiple: true, limit: 2 } })
    const zone = wrapper.find('.rd-fileupload__dragger')
  ...
  it('removes a listed file', async () => {
    ...
    expect(wrapper.find('.rd-fileupload__file').exists()).toBe(true)

    await wrapper.find('[aria-label="删除文件"]').trigger('click')
    await nextTick()
    expect(wrapper.emitted('remove')?.[0]?.[0]).toMatchObject({ name: 'hello.txt' })
    expect(wrapper.find('.rd-fileupload__file').exists()).toBe(false)
  })
  ...
  it('waits for submit when autoUpload is false', async () => {
    ...
    await wrapper.find('.rd-fileupload__toolbar .rd-fileupload__choose').trigger('click')
    await vi.waitFor(() => {
      expect(wrapper.emitted('success')).toBeTruthy()
    })
  ...
  it('renders picture-card add tile and opens the picker', async () => {
    const wrapper = mount(RdFileUpload, { props: { listType: 'picture-card' } })
    expect(wrapper.find('.rd-fileupload__choose').exists()).toBe(false)
    expect(wrapper.find('.rd-fileupload__card--add').exists()).toBe(true)

    const input = wrapper.find('.rd-fileupload__input').element as HTMLInputElement
    const click = vi.spyOn(input, 'click').mockImplementation(() => undefined)
    await wrapper.find('.rd-fileupload__card--add').trigger('click')
    expect(click).toHaveBeenCalled()
  })
  ...
  it('sets directory attributes on the file input', () => {
    const wrapper = mount(RdFileUpload, { props: { directory: true } })
    const input = wrapper.find('.rd-fileupload__input').element as HTMLInputElement
  ...
  it('emits exceed-size for oversized files', async () => {
    const wrapper = mount(RdFileUpload, { props: { maxSize: 4 } })
