...
<template>
  ...
  >
    <div
      ...
      class="rd-tieredmenu__row"
      @mouseenter="openSubmenu(index, item)"
    >
      <div v-if="item.separator" class="rd-tieredmenu__separator" role="separator" />
      <button
        v-else
        type="button"
        class="rd-tieredmenu__item"
        role="menuitem"
      ...
      >
        ...
        <span v-if="item.items?.length" class="rd-tieredmenu__caret" aria-hidden="true">
          <RdIcon name="chevron-right" size="sm" />
        </span>
      </button>
      <div
        v-if="item.items?.length && openIndex === index"
        class="rd-tieredmenu__submenu"
        role="menu"
      >
        <template v-for="(child, childIndex) in item.items" :key="`${child.label ?? 'sep'}-${childIndex}`">
          <div v-if="child.separator" class="rd-tieredmenu__separator" role="separator" />
          <button
            v-else
            type="button"
            class="rd-tieredmenu__item"
  ...
  <Teleport v-else :to="teleportTarget.to" :disabled="teleportTarget.disabled">
    <Transition name="rd-scale-fade">
      <div
        v-if="modelValue"
        ref="root"
        class="rd-tieredmenu rd-tieredmenu--popup"
        :class="{ 'rd-tieredmenu--teleported': teleported }"
