...
describe('wkTabs', () => {
  it('emits the selected tab value', async () => {
    const wrapper = mount(WkTabs, { props: { tabs, modelValue: 'design' } })
  ...
  it('supports arrow-key navigation across enabled tabs', async () => {
    const wrapper = mount(WkTabs, { attachTo: document.body, props: { tabs, modelValue: 'design' } })
  ...
  it('closes a tab and adds a tab', async () => {
    ...
    await wrapper.get('.wk-tabs__close').trigger('click')
    expect(wrapper.emitted('close')?.[0]).toEqual(['a'])
    expect(wrapper.emitted('update:modelValue')?.at(-1)).toEqual(['b'])
    await wrapper.get('.wk-tabs__add').trigger('click')
  ...
  it('links tabs to panels with aria-controls and aria-labelledby', () => {
    ...
    expect(panel.attributes('aria-labelledby')).toBe(`wk-tab-${tabs[0]!.value}`)
