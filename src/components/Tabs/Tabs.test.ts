...
describe('muTabs', () => {
  it('emits the selected tab value', async () => {
    const wrapper = mount(MTabs, { props: { tabs, modelValue: 'design' } })
  ...
  it('supports arrow-key navigation across enabled tabs', async () => {
    const wrapper = mount(MTabs, { attachTo: document.body, props: { tabs, modelValue: 'design' } })
  ...
  it('closes a tab and adds a tab', async () => {
    ...
    await wrapper.get('.m-tabs__close').trigger('click')
    expect(wrapper.emitted('close')?.[0]).toEqual(['a'])
    expect(wrapper.emitted('update:modelValue')?.at(-1)).toEqual(['b'])
    await wrapper.get('.m-tabs__add').trigger('click')
