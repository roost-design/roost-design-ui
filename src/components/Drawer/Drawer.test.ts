...
import RdDrawer from './Drawer.vue'

describe('rdDrawer', () => {
  it('opens with show event and closes with Escape', async () => {
    const wrapper = mount(RdDrawer, {
      attachTo: document.body,
      props: { modelValue: true, header: 'Menu' },
    })
    await nextTick()
    expect(document.body.querySelector('.rd-drawer')).toBeTruthy()
  ...
  it('closes on mask click when dismissable and via close icon', async () => {
    ...
    await nextTick()
    expect(document.body.querySelector('.rd-drawer--right')).toBeTruthy()
    const closeButton = document.body.querySelector('.rd-drawer__close')
    ...
    const masked = mount(RdDrawer, {
      attachTo: document.body,
      props: { modelValue: true, dismissable: false },
    })
    await nextTick()
    document.body.querySelector('.rd-drawer-backdrop')!.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    await nextTick()
  ...
  it('applies width for side drawers', async () => {
    ...
    await nextTick()
    const pane = document.body.querySelector('.rd-drawer') as HTMLElement
  ...
  it('respects closeOnEsc=false and emits close', async () => {
    ...
    document.body.querySelector('.rd-drawer__close')!.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    await nextTick()
