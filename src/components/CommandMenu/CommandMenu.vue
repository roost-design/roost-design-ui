...
<template>
  <Teleport :to="teleportTarget.to" :disabled="teleportTarget.disabled">
    <Transition name="rd-fade">
      <div v-if="modelValue" class="rd-commandmenu-backdrop" @click.self="close">
        <div
          ref="panelRef"
          class="rd-commandmenu"
          role="dialog"
          aria-modal="true"
        ...
        >
          <input
            ref="inputRef"
            v-model="query"
            class="rd-commandmenu__input"
            type="search"
            :placeholder="searchPlaceholder"
            :aria-label="locale.searchCommands"
          >
          <ul class="rd-commandmenu__list" role="listbox">
            <li v-for="(item, index) in filtered" :key="`${item.label}-${index}`" role="presentation">
              <button
                type="button"
                class="rd-commandmenu__item"
                role="option"
                :class="{ 'rd-commandmenu__item--active': index === activeIndex }"
                :aria-selected="index === activeIndex"
                :disabled="item.disabled"
                @click="activate(item)"
                @mouseenter="activeIndex = index"
              >
                <span v-if="iconOf(item)" class="rd-commandmenu__icon" aria-hidden="true">
                  <RdIcon :name="iconOf(item)!" size="sm" />
                </span>
                <span class="rd-commandmenu__label">{{ item.label }}</span>
                <span v-if="item.shortcut" class="rd-commandmenu__shortcut">{{ item.shortcut }}</span>
              </button>
            </li>
            <li v-if="!filtered.length" class="rd-commandmenu__empty">
              {{ locale.noMatch }}
            </li>
          </ul>
        </div>
