...
import RdPickList from './PickList.vue'

describe('rdPickList', () => {
  it('moves selected source items to target', async () => {
    const wrapper = mount(RdPickList, {
      props: { source: ['a', 'b'], target: ['c'] },
    })
    await wrapper.findAll('.rd-picklist__listbox')[0]!.findAll('.rd-picklist__item')[0]!.trigger('click')
  ...
  it('shows empty message in empty lists', () => {
    ...
    const emptyItems = wrapper.findAll('.rd-picklist__empty')
