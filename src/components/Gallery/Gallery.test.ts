...
import MGallery from './Gallery.vue'
...
describe('muGallery', () => {
  it('changes active image via thumb and nav', async () => {
    const wrapper = mount(MGallery, { props: { images, activeIndex: 0 } })
    await wrapper.findAll('.m-gallery__thumb')[2]!.trigger('click')
  ...
  it('renders object images with alt and caption', () => {
    const wrapper = mount(MGallery, {
      props: {
        images: [{ src: 'https://example.com/a.jpg', alt: 'A photo', caption: 'First' }],
        activeIndex: 0,
      },
    })
    expect(wrapper.find('.m-gallery__image').attributes('alt')).toBe('A photo')
    expect(wrapper.find('.m-gallery__caption').text()).toBe('First')
    expect(wrapper.find('.m-gallery__thumb img').attributes('alt')).toBe('A photo')
  })
  ...
  it('supports arrow-key navigation across thumbs with roving tabindex', async () => {
    ...
    const thumbs = () => wrapper.findAll('.m-gallery__thumb')
    ...
    await wrapper.find('.m-gallery__thumbs').trigger('keydown', { key: 'ArrowRight' })
    await nextTick()
    expect(wrapper.emitted('update:activeIndex')?.at(-1)).toEqual([1])
