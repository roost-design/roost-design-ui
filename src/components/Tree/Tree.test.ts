...
describe('rdTree', () => {
  it('expands nodes and selects single key', async () => {
    const wrapper = mount(RdTree, { props: { value, modelValue: null } })
    await wrapper.find('.rd-tree__toggler').trigger('click')
    expect(wrapper.findAll('.rd-tree__label').length).toBeGreaterThan(1)
    await wrapper.findAll('.rd-tree__label')[1]!.trigger('click')
    expect(wrapper.emitted('update:modelValue')?.at(-1)).toEqual(['0-0'])
  })

  it('supports multiple selectionKeys', async () => {
    const wrapper = mount(RdTree, {
      props: { value, selectionMode: 'multiple', selectionKeys: {} },
    })
    await wrapper.find('.rd-tree__label').trigger('click')
  ...
  it('checks nodes with cascade', async () => {
    ...
    await wrapper.find('.rd-checkbox__input').setValue(true)
  ...
  it('shows empty message when filter yields no results', () => {
    ...
    expect(wrapper.find('.rd-tree__message').exists()).toBe(true)
    expect(wrapper.find('.rd-tree__empty-text').text()).toBe('暂无数据')
  })
  ...
  it('loads lazy children without mutating props', async () => {
    ...
    const wrapper = mount(RdTree, { props: { value: nodes, lazy: true, load } })
    await wrapper.find('.rd-tree__toggler').trigger('click')
    await flushPromises()
  ...
  it('supports tree keyboard navigation with roving tabindex', async () => {
    ...
    const tree = wrapper.find('ul.rd-tree')
    const items = () => wrapper.findAll('[role="treeitem"]')
