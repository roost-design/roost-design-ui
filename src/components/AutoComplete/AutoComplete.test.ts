...
import RdAutoComplete from './AutoComplete.vue'

describe('rdAutoComplete', () => {
  it('emits complete and update on input', async () => {
    const wrapper = mount(RdAutoComplete, {
      props: { modelValue: '', suggestions: ['Apple', 'Apricot', 'Banana'] },
      attachTo: document.body,
    })
    await wrapper.find('.rd-autocomplete__input').setValue('Ap')
    expect(wrapper.emitted('update:modelValue')?.at(-1)).toEqual(['Ap'])
    expect(wrapper.emitted('complete')?.at(-1)).toEqual(['Ap'])
    await nextTick()
    const items = document.body.querySelectorAll('.rd-autocomplete__item')
  ...
  it('shows dropdown suggestions when toggled', async () => {
    ...
    await wrapper.find('.rd-autocomplete__dropdown').trigger('click')
    expect(wrapper.findAll('.rd-autocomplete__item')).toHaveLength(2)
  })
  ...
  it('teleports the panel to body by default', async () => {
    ...
    await wrapper.find('.rd-autocomplete__input').trigger('focus')
    await nextTick()
    expect(document.body.querySelector('.rd-autocomplete__panel--teleported')).toBeTruthy()
    wrapper.unmount()
  })
  ...
  it('selects option objects and can clear', async () => {
    ...
    await wrapper.find('.rd-autocomplete__input').trigger('focus')
    await nextTick()
    await wrapper.get('.rd-autocomplete__item').trigger('mousedown')
    expect(wrapper.emitted('update:modelValue')?.at(-1)).toEqual(['apple'])
    await wrapper.setProps({ modelValue: 'apple' })
    await wrapper.get('.rd-autocomplete__clear').trigger('click')
