...
import { h } from 'vue'
import WkInputPassword from './InputPassword.vue'

describe('wkInputPassword', () => {
  it('emits model updates and toggles mask', async () => {
    const wrapper = mount(WkInputPassword, { props: { label: 'Password', id: 'pwd' } })
    ...
    await wrapper.get('.wk-password__toggle').trigger('click')
    expect(wrapper.get('input').attributes('type')).toBe('text')
  })

  it('renders an icon toggle instead of text by default', () => {
    const wrapper = mount(WkInputPassword)
    const toggle = wrapper.get('.wk-password__toggle')
    expect(toggle.text()).toBe('')
    expect(toggle.find('.wk-icon').exists()).toBe(true)
  ...
  it('swaps to the hide icon after revealing the value', async () => {
    const wrapper = mount(WkInputPassword)
    await wrapper.get('.wk-password__toggle').trigger('click')
    expect(wrapper.get('.wk-password__toggle').attributes('aria-label')).toBe('隐藏密码')
    expect(wrapper.get('.wk-password__toggle').attributes('aria-pressed')).toBe('true')
  })

  it('replaces icons via showIcon / hideIcon props', async () => {
    const wrapper = mount(WkInputPassword, { props: { showIcon: 'lock', hideIcon: 'unlock' } })
    expect(wrapper.get('.wk-password__toggle .wk-icon').exists()).toBe(true)
    await wrapper.get('.wk-password__toggle').trigger('click')
    expect(wrapper.get('.wk-password__toggle .wk-icon').exists()).toBe(true)
  })
  ...
  it('replaces icons via showIcon / hideIcon slots', async () => {
    ...
    expect(wrapper.find('.wk-icon').exists()).toBe(false)
    await wrapper.get('.wk-password__toggle').trigger('click')
  ...
  it('shows strength feedback when enabled', async () => {
    ...
    expect(wrapper.get('.wk-password__feedback').text()).toContain('强度')
  })

  it('hides toggle when toggleMask is false', () => {
    const wrapper = mount(WkInputPassword, { props: { toggleMask: false } })
    expect(wrapper.find('.wk-password__toggle').exists()).toBe(false)
    expect(wrapper.find('.wk-password--toggle').exists()).toBe(false)
  })
  ...
  it('clears the value and can peek on mousedown', async () => {
    ...
    expect(wrapper.get('.wk-password-field__count').text()).toBe('6 / 20')
    await wrapper.get('.wk-password__clear').trigger('click')
    expect(wrapper.emitted('update:modelValue')).toEqual([['']])
    expect(wrapper.emitted('clear')).toHaveLength(1)

    const peek = mount(WkInputPassword, {
      props: { modelValue: 'secret', showPasswordOn: 'mousedown' },
    })
    expect(peek.get('input').attributes('type')).toBe('password')
    await peek.get('.wk-password__toggle').trigger('mousedown')
