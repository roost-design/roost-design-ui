...
import RdTreeTable from './TreeTable.vue'
...
describe('rdTreeTable', () => {
  it('renders columns and expands children', async () => {
    const wrapper = mount(RdTreeTable, { props: { columns, value } })
    expect(wrapper.text()).toContain('Applications')
    expect(wrapper.text()).not.toContain('Vue')
    await wrapper.find('.rd-treetable__toggler').trigger('click')
  ...
  it('supports controlled expandedKeys', async () => {
    const wrapper = mount(RdTreeTable, {
      props: {
        columns,
        value,
        expandedKeys: { '0': true },
  ...
  it('shows empty message when value is empty', () => {
    const wrapper = mount(RdTreeTable, { props: { columns, value: [] } })
    expect(wrapper.find('.rd-treetable__message').exists()).toBe(true)
    expect(wrapper.find('.rd-treetable__empty-text').text()).toBe('暂无数据')
  })
  ...
  it('renders expansion slot content when expanded', async () => {
    ...
    expect(wrapper.find('.rd-treetable__expansion-cell').exists()).toBe(true)
  })
})
