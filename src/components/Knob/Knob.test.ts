...
describe('rdKnob', () => {
  ...
  it('clamps with ArrowDown at max boundary', async () => {
    const wrapper = mount(RdKnob, { props: { modelValue: 90, min: 0, max: 90, step: 5 } })
  ...
  it('steps down with ArrowDown', async () => {
    const wrapper = mount(RdKnob, { props: { modelValue: 40, step: 10 } })
    await wrapper.trigger('keydown', { key: 'ArrowDown' })
    expect(wrapper.emitted('update:modelValue')?.at(-1)).toEqual([30])
  })

  it('supports diameter prop over the deprecated size alias', () => {
    const wrapper = mount(RdKnob, { props: { modelValue: 0, diameter: 160 } })
