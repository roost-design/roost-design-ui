...
describe('message API', () => {
  ...
  it('applies host placement from config', async () => {
    ...
    await nextTick()
    expect(document.body.querySelector('.m-message-host--bottom-right')).toBeTruthy()
  })
  ...
  it('renders VNode content from h()', async () => {
    ...
    await nextTick()
    expect(document.body.querySelector('.m-message__content strong')?.textContent).toBe('Rich node')
  })
  ...
  it('pauses auto-close while hovered', async () => {
    ...
    await nextTick()
    const node = document.body.querySelector('.m-message')
