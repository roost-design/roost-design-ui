...
import RdVirtualScroller from './VirtualScroller.vue'

describe('rdVirtualScroller', () => {
  it('renders a window of items', () => {
    const items = Array.from({ length: 100 }, (_, i) => `Item ${i}`)
    const wrapper = mount(RdVirtualScroller, {
  ...
  it('updates window on scroll', async () => {
    ...
    const root = wrapper.find('.rd-virtualscroller')
