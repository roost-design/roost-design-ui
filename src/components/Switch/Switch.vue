...
const rootClass = computed(() => [
  'm-switch',
  `m-switch--${sizeClass.value}`,
  {
    'm-switch--disabled': isDisabled.value,
    'm-switch--invalid': props.invalid,
...
onMounted(() => {
  if (import.meta.env.DEV && !props.label && !slots.default) {
    console.warn('[MSwitch] Provide `label` prop or default slot for an accessible name.')
  }
})
</script>
...
<template>
  <label :class="rootClass" :for="resolvedInputId">
    ...
    <span class="m-switch__track" aria-hidden="true">
      <span v-if="stateText" class="m-switch__text">{{ stateText }}</span>
      <span class="m-switch__thumb">
        <span v-if="loading" class="m-switch__spinner" />
      </span>
    </span>
    <span v-if="label || $slots.default" class="m-switch__label"><slot>{{ label }}</slot></span>
  </label>
</template>
