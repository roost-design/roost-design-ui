...
function paddingStyle(depth: number) {
  ...
  return { paddingInlineStart: `calc(var(--rd-menu-root-indent) + ${depth} * var(--rd-menu-indent))` }
}
...
function focusEntryByKey(key: string | null) {
  ...
  const nodes = root.value.querySelectorAll<HTMLElement>('[data-rd-menu-key]')
  for (const node of nodes) {
    if (node.dataset.rdMenuKey === key) {
...
const menuClass = computed(() => [
  'rd-menu',
  {
    'rd-menu--popup': props.popup,
    'rd-menu--teleported': teleported.value,
    'rd-menu--collapsed': props.collapsed,
    'rd-menu--embedded': embedded.value,
    'rd-menu--horizontal': props.mode === 'horizontal',
    'rd-menu--inverted': props.inverted,
  },
])

const menuStyle = computed(() => ({
  '--rd-menu-indent': `${props.indent / 16}rem`,
  '--rd-menu-root-indent': `${props.rootIndent / 16}rem`,
}))
...
<template>
  ...
  <div v-else-if="hasTriggerSlot" class="rd-menu-popup">
    <div ref="triggerEl" class="rd-menu-popup__anchor">
      <slot />
    </div>
