...
describe('muMenu', () => {
  it('invokes command on item click', async () => {
    ...
    const items = wrapper.findAll('.m-menu__item-content')
    await items[0]!.trigger('click')
    expect(command).toHaveBeenCalledOnce()
    await items[1]!.trigger('click')
    expect(command).toHaveBeenCalledOnce()
    expect(wrapper.find('.m-menu__separator').exists()).toBe(true)
  })
  ...
  it('hides popup until modelValue is true', async () => {
    ...
    expect(wrapper.find('.m-menu').exists()).toBe(false)
    await wrapper.setProps({ modelValue: true })
    expect(wrapper.find('.m-menu').exists()).toBe(true)
  })
  ...
  it('teleports popup menu to body by default', async () => {
    ...
    await nextTick()
    const menu = document.body.querySelector('.m-menu--teleported') as HTMLElement | null
  ...
  it('positions popup menu relative to the default-slot trigger', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.m-menu-popup__anchor').exists()).toBe(true)
    const menu = document.body.querySelector('.m-menu--teleported') as HTMLElement | null
    expect(menu).toBeTruthy()
    expect(menu?.style.top).not.toBe('')
  ...
  it('nests items, expands submenu, and marks selectedKey', async () => {
    ...
    expect(wrapper.find('.m-menu__submenu').exists()).toBe(true)
    expect(wrapper.get('.m-menu__item-content--selected').text()).toContain('Save')
    await wrapper.get('.m-menu__item-content--selected').trigger('click')
  ...
  it('auto-expands path when selectedKey changes', async () => {
    ...
    expect(wrapper.find('.m-menu__submenu').exists()).toBe(false)
  ...
  it('uses accordion to keep one top-level group open', async () => {
    ...
    const parents = wrapper.findAll('.m-menu__item--submenu > .m-menu__item-content')
    await parents[0]!.trigger('click')
    expect(wrapper.findAll('.m-menu__submenu')).toHaveLength(1)
  ...
  it('marks parent as child-active when nested item selected', async () => {
    ...
    expect(wrapper.find('.m-menu__item--submenu .m-menu__item-content--child-active').exists()).toBe(true)
  })
  ...
  it('opens horizontal submenu in flyout instead of inline expand', async () => {
    ...
    expect(wrapper.find('.m-menu--horizontal').exists()).toBe(true)
    expect(wrapper.find('.m-menu__submenu').exists()).toBe(false)
    await wrapper.get('.m-menu__item-content').trigger('click')
    await nextTick()
    expect(document.body.querySelector('.m-menu--flyout')).toBeTruthy()
    wrapper.unmount()
  })
  ...
  it('supports vertical keyboard navigation with roving tabindex', async () => {
    ...
    const root = wrapper.get('.m-menu')
    const contentByText = (text: string) =>
      wrapper.findAll('.m-menu__item-content').find((node) => node.text().includes(text))!
  ...
  it('marks the selected leaf with aria-current', () => {
    ...
    const save = wrapper.get('.m-menu__item-content--selected')
  ...
  it('closes popup on Escape and returns focus to the trigger', async () => {
    ...
    await nextTick()
    await wrapper.get('.m-menu').trigger('keydown', { key: 'Escape' })
