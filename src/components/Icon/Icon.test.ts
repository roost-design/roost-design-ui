...
import { h } from 'vue'
import WkIcon from './Icon.vue'
import { iconNames, isIconName } from './icons'

describe('wkIcon', () => {
  it('hides decorative icons and labels informative ones', () => {
    const decorative = mount(WkIcon, { props: { name: 'search' } })
    const informative = mount(WkIcon, { props: { name: 'trash', label: '删除项目' } })
  ...
  it('maps size aliases to small/normal/large classes', () => {
    expect(mount(WkIcon, { props: { name: 'check', size: 'sm' } }).classes()).toContain('wk-icon--small')
    expect(mount(WkIcon, { props: { name: 'check', size: 'large' } }).classes()).toContain('wk-icon--large')
    expect(mount(WkIcon, { props: { name: 'check', size: 'md' } }).classes()).toContain('wk-icon--normal')
  })
  ...
  it('exposes a registry of system icons', () => {
    ...
    expect(mount(WkIcon, { props: { name: 'loader' } }).classes()).toContain('wk-icon--spin')
  })

  it('prefers the default slot over name for custom icons', () => {
    const wrapper = mount(WkIcon, {
