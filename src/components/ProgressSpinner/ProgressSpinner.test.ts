...
import WkProgressSpinner from './ProgressSpinner.vue'

describe('wkProgressSpinner', () => {
  it('renders SVG circle with defaults', () => {
    const wrapper = mount(WkProgressSpinner)
    expect(wrapper.classes()).toContain('wk-progress-spinner')
    expect(wrapper.find('.wk-progress-spinner__circle').exists()).toBe(true)
  ...
  it('wraps content and respects show', () => {
    const hidden = mount(WkProgressSpinner, {
      props: { show: false, description: 'Saving' },
      slots: { default: '<p>Form</p>' },
    })
    expect(hidden.text()).toContain('Form')
    expect(hidden.find('.wk-progress-spinner-wrap__overlay').exists()).toBe(false)

    const shown = mount(WkProgressSpinner, {
      props: { show: true, description: 'Saving' },
      slots: { default: '<p>Form</p>' },
    })
    expect(shown.get('.wk-progress-spinner-wrap__overlay').text()).toContain('Saving')
  })
  ...
  it('marks wrapped content inert while loading overlay is visible', () => {
    ...
    expect(wrapper.get('.wk-progress-spinner-wrap__content').attributes('inert')).toBeDefined()
    expect(wrapper.get('.wk-progress-spinner-wrap').attributes('aria-busy')).toBe('true')
  })
})
