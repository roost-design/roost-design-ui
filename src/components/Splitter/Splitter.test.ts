...
import MSplitter from './Splitter.vue'
...
describe('muSplitter', () => {
  it('renders named panel slots', () => {
    ...
    expect(wrapper.find('.m-splitter__gutter').exists()).toBe(true)
  })
  ...
  it('supports direction alias for layout', () => {
    ...
    expect(wrapper.classes()).toContain('m-splitter--vertical')
  })
  ...
  it('resizes on gutter drag via window mousemove', async () => {
    ...
    expect(wrapper.findAll('.m-splitter__panel')[0]!.attributes('style')).toContain('30%')
    wrapper.unmount()
  })
  ...
  it('supports keyboard resize on the gutter', async () => {
    ...
    await wrapper.get('.m-splitter__gutter').trigger('keydown', { key: 'ArrowRight' })
  ...
  it('supports ratio size mode', () => {
    ...
    expect(wrapper.findAll('.m-splitter__panel')[0]!.attributes('style')).toContain('35%')
  })
  ...
  it('does not drag when disabled', async () => {
    ...
    await wrapper.get('.m-splitter__gutter').trigger('mousedown', { clientX: 100, clientY: 50 })
