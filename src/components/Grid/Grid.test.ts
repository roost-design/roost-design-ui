...
import { h } from 'vue'
import WkGrid from './Grid.vue'
import WkGridItem from './GridItem.vue'

describe('wkGrid', () => {
  it('renders a css grid with items', () => {
    const wrapper = mount(WkGrid, {
      props: { cols: 12, xGap: 8, yGap: 8 },
      slots: {
        default: () => [
          h(WkGridItem, { span: 6 }, () => 'A'),
          h(WkGridItem, { span: 6 }, () => 'B'),
        ],
      },
    })
    expect(wrapper.classes()).toContain('wk-grid')
    expect(wrapper.element.style.gridTemplateColumns).toContain('repeat(12')
    expect(wrapper.findAll('.wk-grid-item')).toHaveLength(2)
  })
  ...
  it('hides overflowing items when collapsed', async () => {
    const wrapper = mount(WkGrid, {
      ...
      slots: {
        default: () => [
          h(WkGridItem, { span: 2 }, () => 'A'),
          h(WkGridItem, { span: 2 }, () => 'B'),
          h(WkGridItem, { span: 2 }, () => 'C'),
        ],
      },
    })
    await wrapper.vm.$nextTick()
    const items = wrapper.findAll('.wk-grid-item')
  ...
  it('applies offset into grid-column span like naive-ui', async () => {
    const wrapper = mount(WkGrid, {
      ...
      slots: {
        default: () => [
          h(WkGridItem, { span: 1, offset: 2 }, () => 'A'),
          h(WkGridItem, { span: 1, offset: 1 }, () => 'B'),
        ],
      },
    })
    await wrapper.vm.$nextTick()
  ...
  it('resolves item span responsive strings when itemResponsive is on', async () => {
    const wrapper = mount(WkGrid, {
      ...
      slots: {
        default: () => [h(WkGridItem, { span: '1 m:4' }, () => 'A')],
      },
      attachTo: document.body,
    })
    Object.defineProperty(wrapper.get('.wk-grid').element, 'clientWidth', {
      configurable: true,
      get: () => 1200,
    })
    await wrapper.setProps({ xGap: 8 })
    await wrapper.vm.$nextTick()
    expect(wrapper.find('.wk-grid-item').attributes('style')).toContain('grid-column: span 4 / span 4')
    wrapper.unmount()
  })
