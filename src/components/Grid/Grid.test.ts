...
describe('muGrid', () => {
  it('renders a css grid with items', () => {
    const wrapper = mount(MGrid, {
      ...
      slots: {
        default: () => [
          h(MGridItem, { span: 6 }, () => 'A'),
          h(MGridItem, { span: 6 }, () => 'B'),
        ],
      },
    })
    expect(wrapper.classes()).toContain('m-grid')
    expect(wrapper.element.style.gridTemplateColumns).toContain('repeat(12')
    expect(wrapper.findAll('.m-grid-item')).toHaveLength(2)
  })
  ...
  it('hides overflowing items when collapsed', async () => {
    const wrapper = mount(MGrid, {
      ...
      slots: {
        default: () => [
          h(MGridItem, { span: 2 }, () => 'A'),
          h(MGridItem, { span: 2 }, () => 'B'),
          h(MGridItem, { span: 2 }, () => 'C'),
        ],
      },
    })
    await wrapper.vm.$nextTick()
    const items = wrapper.findAll('.m-grid-item')
  ...
  it('applies offset into grid-column span like naive-ui', async () => {
    const wrapper = mount(MGrid, {
      ...
      slots: {
        default: () => [
          h(MGridItem, { span: 1, offset: 2 }, () => 'A'),
          h(MGridItem, { span: 1, offset: 1 }, () => 'B'),
        ],
      },
    })
    await wrapper.vm.$nextTick()
  ...
  it('resolves item span responsive strings when itemResponsive is on', async () => {
    const wrapper = mount(MGrid, {
      ...
      slots: {
        default: () => [h(MGridItem, { span: '1 m:4' }, () => 'A')],
      },
      attachTo: document.body,
    })
    Object.defineProperty(wrapper.get('.m-grid').element, 'clientWidth', {
      configurable: true,
      get: () => 1200,
    })
    await wrapper.setProps({ xGap: 8 })
    await wrapper.vm.$nextTick()
    expect(wrapper.find('.m-grid-item').attributes('style')).toContain('grid-column: span 4 / span 4')
    wrapper.unmount()
  })
