...
import RdOrderList from './OrderList.vue'

describe('rdOrderList', () => {
  it('reorders selected item down', async () => {
    const wrapper = mount(RdOrderList, { props: { modelValue: ['a', 'b', 'c'] } })
    await wrapper.findAll('.rd-orderlist__item')[0]!.trigger('click')
  ...
  it('reorders selected item up', async () => {
    const wrapper = mount(RdOrderList, { props: { modelValue: ['a', 'b', 'c'] } })
    await wrapper.findAll('.rd-orderlist__item')[2]!.trigger('click')
  ...
  it('reorders via native drag and drop from the handle', async () => {
    const wrapper = mount(RdOrderList, { props: { modelValue: ['a', 'b', 'c'] } })
    const items = wrapper.findAll('.rd-orderlist__item')
    const from = items[0]!
    const to = items[2]!
    const dataTransfer = {
      effectAllowed: 'none',
    ...
    await from.find('.rd-orderlist__handle').trigger('pointerdown')
  ...
  it('does not start drag without arming the handle', async () => {
    const wrapper = mount(RdOrderList, { props: { modelValue: ['a', 'b', 'c'] } })
    const from = wrapper.findAll('.rd-orderlist__item')[0]!
    const dataTransfer = {
      effectAllowed: 'none',
      dropEffect: 'none',
      setData: () => undefined,
  ...
  it('moves selection with arrow keys and reorders with Ctrl+Arrow', async () => {
    const wrapper = mount(RdOrderList, {
    ...
    const list = wrapper.get('.rd-orderlist__list')
  ...
  it('shows empty message when list is empty', () => {
    const wrapper = mount(RdOrderList, { props: { modelValue: [] } })
    expect(wrapper.find('.rd-orderlist__message').exists()).toBe(true)
    expect(wrapper.find('.rd-orderlist__empty-text').text()).toBe('暂无数据')
  })
})
