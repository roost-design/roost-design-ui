...
import MRating from './Rating.vue'

describe('muRating', () => {
  it('renders stars and emits selection', async () => {
    const wrapper = mount(MRating, { props: { modelValue: 2, stars: 5 } })
    const stars = wrapper.findAll('.m-rating__star')
    expect(stars).toHaveLength(5)
    expect(wrapper.findAll('.m-rating__star--on')).toHaveLength(2)
  ...
  it('cancels rating when cancel is enabled', async () => {
    const wrapper = mount(MRating, { props: { modelValue: 3, cancel: true } })
    await wrapper.get('.m-rating__cancel').trigger('click')
    expect(wrapper.emitted('update:modelValue')).toEqual([[0]])
  })

  it('does not emit while readonly or disabled', async () => {
    const readonly = mount(MRating, { props: { modelValue: 1, readonly: true } })
    await readonly.findAll('.m-rating__star')[2]!.trigger('click')
    expect(readonly.emitted('update:modelValue')).toBeUndefined()

    const disabled = mount(MRating, { props: { modelValue: 1, disabled: true, cancel: false } })
    expect(disabled.find('.m-rating__cancel').exists()).toBe(false)
    await disabled.findAll('.m-rating__star')[2]!.trigger('click')
    expect(disabled.emitted('update:modelValue')).toBeUndefined()
  })

  it('supports half stars and allowClear alias', async () => {
    const wrapper = mount(MRating, { props: { modelValue: 1, allowHalf: true, allowClear: false } })
    expect(wrapper.find('.m-rating__cancel').exists()).toBe(false)
    const star = wrapper.findAll('.m-rating__star')[1]!
    await star.trigger('click', { clientX: 0 })
    expect(wrapper.emitted('update:modelValue')?.at(-1)?.[0]).toBeTypeOf('number')
    expect(wrapper.get('.m-rating').classes()).toContain('m-rating--half')
  })

  it('exposes slider semantics with label and value text', () => {
    const wrapper = mount(MRating, { props: { modelValue: 3, stars: 5 } })
  ...
  it('does not respond to keys when readonly', async () => {
    const wrapper = mount(MRating, { props: { modelValue: 2, readonly: true } })
