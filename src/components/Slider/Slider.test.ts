...
import RdSlider from './Slider.vue'

describe('rdSlider', () => {
  it('emits single value updates', async () => {
    const wrapper = mount(RdSlider, { props: { modelValue: 20, min: 0, max: 100 } })
  ...
  it('marks disabled state', () => {
    const wrapper = mount(RdSlider, { props: { disabled: true } })
    expect(wrapper.get('.rd-slider').classes()).toContain('rd-slider--disabled')
  ...
  it('renders marks and vertical layout', () => {
    ...
    expect(wrapper.get('.rd-slider').classes()).toEqual(
      expect.arrayContaining(['rd-slider--vertical', 'rd-slider--tooltip']),
    )
