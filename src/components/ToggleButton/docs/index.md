...
<template>
  <RdToggleButton v-model="on" on-label="开启" off-label="关闭" />
</template>
```
...
<template>
  <div style="display:flex;flex-wrap:wrap;gap:0.75rem;align-items:center">
    <RdToggleButton v-model="a" size="small" on-label="小" off-label="小" />
    <RdToggleButton v-model="b" on-label="默认" off-label="默认" />
    <RdToggleButton v-model="c" size="large" on-label="大" off-label="大" />
  </div>
</template>
```
