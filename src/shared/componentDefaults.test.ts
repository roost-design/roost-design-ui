...
import { mergeMConfig } from './config'

describe('componentDefaults', () => {
  it('reads unprefixed keys and M aliases', () => {
    expect(getComponentDefault({ Input: { size: 'small' } }, 'MInput', 'size')).toBe('small')
    expect(getComponentDefaults({ MSpace: { size: 12 } }, 'Space')).toEqual({ size: 12 })
    expect(getComponentDefaults({ Input: { size: 'small' }, MInput: { clearable: true } }, 'Input')).toEqual({
      size: 'small',
      clearable: true,
    })
  })
