...
| **RdMessage 组件** | 页面内嵌条，不自动消失 | `<RdMessage severity="error">…</RdMessage>` | 表单/认证区**常驻**错误、警告 |

## 决策树（按顺序判断）

```
用户操作完成，需要即时反馈？
├─ 否 → 不需要 Message/Toast（可能用 RdConfirmDialog / 字段 errorMessage）
└─ 是 → 错误需留在表单区域直到用户修正？
    ├─ 是 → <RdMessage> 或字段级 invalid / errorMessage
...
```vue
<RdMessage v-if="error" severity="error" :closable="false">
  {{ error }}
</RdMessage>
```
...
| 表单校验失败只弹 Toast | 字段 `errorMessage` 或 `<RdMessage>` |
| 删除确认用 Message/Toast | `RdConfirmDialog` |
...
| `<RdMessage>` | 页面内 `el-alert` / 表单顶部错误条 |
