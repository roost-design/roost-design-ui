#!/usr/bin/env node
/**
 * Scan CSS files for var(--rd-*) references that are never defined.
 ...
 * (e.g. { '--rd-button-color': props.color }) also count as defined.
...
const COMMENT = /\/\*[\s\S]*?\*\//g
const REFERENCE = /var\(\s*(--rd-[\w-]+)/g
const DEFINITION = /(?<![\w-])(--rd-[\w-]+)\s*:/g
const RUNTIME_PROVIDED = /['"](--rd-[\w-]+)['"]\s*:/g
...
if (violations.length) {
  console.error(`Found ${violations.length} reference(s) to undefined --rd-* token(s):\n`)
...
console.log(`No undefined --rd-* token references found (${defined.size} tokens defined).`)
