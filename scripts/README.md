...
`@roost-design/ui` 的版本与 CHANGELOG 在发版时由你交互确认；日常按 [提交规范](../docs/COMMIT_CONVENTION.md) 提交即可。更完整的维护说明见 [UI 开发](../docs/ui-development.zh-CN.md)（[English](../docs/ui-development.md)）与 [开发指南](../docs/DEVELOPMENT.zh-CN.md)（[English](../docs/DEVELOPMENT.md)）。

默认情况下，一次 `pnpm release` 会同时发布 `@roost-design/ui` 与 `@roost-design/ui-mcp`（版本对齐）。
...
| 5 | `pnpm release:publish` | 发布 `@roost-design/ui` 与 `@roost-design/ui-mcp` |
...
| `pnpm release:mcp` | 单独发布 `@roost-design/ui-mcp`（同步 UI 版本 → 构建 → commit → publish） |
...
提交信息格式：`release: @roost-design/ui / @roost-design/ui-mcp v{version}`（`--no-mcp` 时仅 UI）。
